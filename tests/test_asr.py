...
A fake provider replaces Deepgram so no key or network is needed; the test
drives a full push-to-talk turn over the WebSocket and asserts the protocol
ordering: ``transcript_partial`` then exactly one ``transcript_final`` ahead of
``turn_complete``.
"""
...
class FakeASRSession(ASRSession):
    ...
    async def send_audio(self, pcm: bytes) -> None:
        if not self._emitted_partial:
            ...
            await self._on_transcript(Transcript(text="hello", is_final=False))
...
class FakeASRProvider(ASRProvider):
    async def start(self, on_transcript: OnTranscript) -> ASRSession:
        return FakeASRSession(on_transcript)


@pytest.fixture
def fake_asr(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("server.ws.echo.get_asr_provider", lambda _settings: FakeASRProvider())
...
def test_turn_emits_partial_then_single_final(fake_asr: None) -> None:
...
def test_turn_partial_arrives_before_final(fake_asr: None) -> None:
